...
import com.b3.cripto.config.KafkaSseConsumer;
import com.b3.cripto.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/cripto-kafka")
public class CriptoKafkaResource {

    private final Logger log = LoggerFactory.getLogger(CriptoKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public CriptoKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
